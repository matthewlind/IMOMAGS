...
add_action('init', 'ecpt_add_thumbnail_support');
...
function ecpt_get_date($date) {
	...
	return date($ecpt_options['date_format'], $date);
}
...
function ecpt_get_field_type($field_id) {
	...
	$type = $wpdb->get_results($wpdb->prepare("SELECT type FROM " . $ecpt_db_meta_fields_name . " WHERE name='" . $field_id . "';"));
	if($type) {
		return $type[0]->type;
	}
	return false;
