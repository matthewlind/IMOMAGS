	...
	if($metabox) {
		...
		if(is_string($type)) {
			if($type == 'all') { // display all meta fields
				$fields = $wpdb->get_results("SELECT * FROM " . $ecpt_db_meta_fields_name . " WHERE parent='" . $metabox . "' ORDER BY list_order;");
			} else {
				$fields = $wpdb->get_results("SELECT * FROM " . $ecpt_db_meta_fields_name . " WHERE parent='" . $metabox . "' AND type='" . $type . "' ORDER BY list_order;");
			}
		} else {
			$types = ''; $i = 0;
			foreach($type as $t) {
			...
			$fields = $wpdb->get_results("SELECT * FROM " . $ecpt_db_meta_fields_name . " WHERE parent='" . $metabox . "' AND (" . $types . ") ORDER BY list_order;");
		}
			...
			foreach( $fields as $key => $field) {
				...
				switch($field_type) {
					...
					case 'text' :
						if($value) {
							$display .= '<li class="ecpt_field ecpt_' . $field->type . '_field" id="ecpt_' . $field->name . '">';
								...
								$display .= '<div class="ecpt_field_content">' . ecpt_filter_links($value, $blank) . '</div>';
							$display .= '</li>';
						}
						break;
						...
						if($value) {
							$display .= '<li class="ecpt_field ecpt_' . $field->type . '_field" id="ecpt_' . $field->name . '">';
								...
								if($field->rich_editor == 1) {
									$display .= '<div class="ecpt_field_content">' . $value . '</div>';
								} else {
									$display .= '<div class="ecpt_field_content">' . ecpt_filter_links($value, $blank) . '</div>';
								}
							$display .= '</li>';
					...
					case 'upload' :
						if($value) {
							$display .= '<li class="ecpt_field ecpt_' . $field->type . '_field" id="ecpt_' . $field->name . '">';
								$display .= '<span class="ecpt_field_name">' . $field->nicename . ': </span>';
								if($descriptions) {
									$display .= '<div class="ecpt_field_desc">' . $field->description . '</div>';
								}
					...
					case 'date' :
						if($value) {
							$display .= '<li class="ecpt_field ecpt_' . $field->type . '_field" id="ecpt_' . $field->name . '">';
								...
								$display .= '<span class="ecpt_field_content">' . date($dateformat, $value) . '</span>';
							$display .= '</li>';
						}
						break;
					...
					case 'checkbox' :
						$display .= '<li class="ecpt_field ecpt_' . $field->type . '_field" id="ecpt_' . $field->name . '">';
							...
							if($value) { $display .= __('yes'); } else { $display .= __('no'); }
							$display .= '</span>';
						$display .= '</li>';
				...
				if(has_filter('ecpt_display_fields')) {
					$display = apply_filters('ecpt_display_fields', $display, $value, $field);	
				}	
...
function ecpt_auto_display_meta($content) {
	...
	if(isset($ecpt_options['auto_display_meta_fields_' . $post->post_type])) {
		if(is_singular() && $ecpt_options['auto_display_meta_fields_' . $post->post_type] == true) {
		
			$metaboxes = $wpdb->get_results("SELECT name FROM " . $ecpt_db_meta_name . " WHERE page='" . $post->post_type . "';");
			if($metaboxes) {
				foreach($metaboxes as $metabox) {
					//ecpt_display_meta($metabox, $type = 'all', $descriptions = true, $images = true, $dateformat = 'F j, Y')
					
					if($ecpt_options['meta_fields_' . $post->post_type . '_desc'] == true)
						$desc = true;
						
					// setup the field types to display
					$types = array();	
					if($ecpt_options['meta_fields_' . $post->post_type . '_text'] == true)	
						$types[] = 'text';
					if($ecpt_options['meta_fields_' . $post->post_type . '_textarea'] == true)	
						$types[] = 'textarea';
					if($ecpt_options['meta_fields_' . $post->post_type . '_date'] == true)	
						$types[] = 'date';
					if($ecpt_options['meta_fields_' . $post->post_type . '_slider'] == true)	
						$types[] = 'slider';
					if($ecpt_options['meta_fields_' . $post->post_type . '_select'] == true)	
						$types[] = 'select';
					if($ecpt_options['meta_fields_' . $post->post_type . '_radio'] == true)	
						$types[] = 'radio';
					if($ecpt_options['meta_fields_' . $post->post_type . '_checkbox'] == true)	
						$types[] = 'checkbox';
					if($ecpt_options['meta_fields_' . $post->post_type . '_upload'] == true)	
						$types[] = 'upload';
					if($ecpt_options['meta_fields_' . $post->post_type . '_repeatable'] == true)	
						$types[] = 'repeatable';
					...
					if($ecpt_options['meta_fields_' . $post->post_type . '_upload_image'] == true)	
						$images = true;
					else 
						$images = false;
						
					// check whether repeatable images should be rendered
					if($ecpt_options['meta_fields_' . $post->post_type . '_repeatable_upload_image'] == true)	
...
add_action('the_content', 'ecpt_auto_display_meta');
