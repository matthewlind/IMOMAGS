...
/*
Plugin Name: SV News widget 
Plugin URI: http://www.sitebase.be
Description: Show news from SportsmenVote feed in a widget
Author: Sitebase
Version: 1.1
...
class svNewsWidget extends WpFramework_Base_0_5 {
		
	const NAME = 'SV News Widget';
	const NAME_SLUG = 'sv-news-widget';
	...
	public function svNewsWidget() {
		parent::__construct();
		$widget_ops = array('description' => 'Show news from sportsmen vote feed in a widget.' );
	...
	public function action_init() {
		add_shortcode('svnewswidget', array($this, 'shortcode'));
	...
	public function widget($args, $instance) {
		...
		$cache_key = 'sv_news_widget_' . md5(implode('_', $data));
		...
		if($data['use_ajax'] == 'true') {
			...
			$_SESSION['svnewswidget']['ajax'][$data['widget_id']]['instance'] = serialize($instance);
			$_SESSION['svnewswidget']['ajax'][$data['widget_id']]['args'] = serialize($args);
		}
	...
	public function refresh() {
		...
		$args = array();
		$instance = array();
		if(isset($_SESSION['svnewswidget'])){
			$args = unserialize($_SESSION['svnewswidget']['ajax'][$_POST['id']]['args']);
			$instance = unserialize($_SESSION['svnewswidget']['ajax'][$_POST['id']]['instance']);
		}
...
add_action('widgets_init', create_function('', 'register_widget("svNewsWidget");'));