...
add_action('init', 'imo_ga_vault_init');
register_activation_hook( __FILE__, 'imo_ga_vault_activate' );
add_action('admin_init', 'imo_ga_vault_admin_script');

function imo_ga_vault_admin_script() {
  wp_register_script('jquery-chosen', plugins_url('chosen/chosen.jquery.min.js', __FILE__), array('jquery'), '1.0');
	wp_register_script('jquery-imo-slider-admin', plugins_url('imo-ga-vault-admin.js', __FILE__), array('jquery','jquery-chosen'), '1.0');
...
function imo_ga_vault_init() {
	$labels = array(
	  ...
	  $args = array(
		...
		'show_ui' => true, 
		'query_var' => true,
		'rewrite' => array('slug' => 'ga-vault', 'with_front' => FALSE),
...
function imo_ga_vault_flush() {
	
	flush_rewrite_rules();
	
}

register_activation_hook( __FILE__, 'imo_ga_vault_flush' );
register_deactivation_hook( __FILE__, 'imo_ga_vault_flush' );
...
add_action( 'add_meta_boxes', 'imo_ga_vault_add_custom_box' );
add_action( 'save_post', 'imo_ga_vault_save_postdata' );

/* Adds a box to the main column on the Post and Page edit screens */
function imo_ga_vault_add_custom_box() {
    add_meta_box( 
        'imo_ga_vault_sectionid',
        __( 'Choose caption', 'imo_ga_vault_textdomain' ),
        'imo_ga_vault_inner_custom_box',
        'imo_ga_vault',
        'normal',
        'high' 
    );
...
function imo_ga_vault_inner_custom_box( $post ) {

  // Use nonce for verification
  wp_nonce_field( plugin_basename( __FILE__ ), 'imo_ga_vault_noncename' );
  ...
  //echo '<input type="text" id="imo_ga_vault_caption_id" name="imo_ga_vault_caption_id" placeholder="Select a caption" size="25" ' . $valueTag . ' />';
  echo '<select id="imo_ga_vault_caption_id" class="chzn-select" name="imo_ga_vault_caption_id" placeholder="Select a Caption" style="width:500px;" />';
...
function imo_ga_vault_save_postdata( $post_id ) {
  ...
  if ( !wp_verify_nonce( $_POST['imo_ga_vault_noncename'], plugin_basename( __FILE__ ) ) )
      return;
  ...
  $mydata = $_POST['imo_ga_vault_caption_id'];
