<?php
	...
	/* Hook: new user from backend */
	add_action('wp_login', 'userpro_new_user_display_name', 10, 2);
	function userpro_new_user_display_name($user_login, $user){
		global $userpro;
		if ( !$userpro->has('display_name', $user->ID) ) {
			$userpro->set_default_display_name($user_id, $user_login);
		}
	}
