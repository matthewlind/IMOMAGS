 ...
 * @return the content
 */ 
function nggShowRelatedGallery($taglist, $maxImages = 0, $match_all_tags = 0) {
    
    $ngg_options = nggGallery::get_option('ngg_options');
    
    // get now the related images
    $picturelist = nggTags::find_images_for_tags($taglist, 'RAND', $match_all_tags);
...
function get_objects_in_term_ex( $term_ids, $taxonomies, $args = array() ) {
    ...
    $sql_query .= " AND tr.object_id IN (SELECT object_id FROM wp_term_relationships WHERE term_taxonomy_id = '". $term_id ."')" ;
