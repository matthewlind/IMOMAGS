	...
	function madtabs() {
		...
		var activehash = "madtabs-"+(madnessround);
		//console.log(activehash);	
		jQuery('.tabcontent').hide();
		jQuery('.rounds li:nth-child('+parseInt(madnessround-1)+')').addClass("ui-tabs-selected");
		jQuery('.'+activehash+'').show();
	}
			...
			success: function(resp, status, jqxhr) {
				if(ismobile) {
					if(resp.type == "finals"){
						...
						jQuery(".match217").html(writeGAMBracket(finalone));
						jQuery(".match218").html(writeGAMBracket(finaltwo));
			...
			jQuery.magnificPopup.open({
				...
				callbacks: {
					markupParse: function(template, values, item) {
						...
						popads[0] = 'GA-MAdness-popup-358x90.jpg';
