		...
		case 1:
			...
			foreach ( range( $start_page, $end_page ) as $i ) {
				if ( $i == $paged && !empty( $options['current_text'] ) ) {
					...
					$out .= "<span class='current'>".((($current_page_text-1) * $posts_per_page) + 1 )." - ".($current_page_text * $posts_per_page)." </span>";
					$timeline = 'larger';
				} else {
			...
			if ( $paged < $total_pages )
				$out .="<a class='next-link' style='display:none;' href='".$instance->get_url($paged + 1)."'>next</a>";
			break;
...
class PageNavi_Call {
	...
	function get_single( $page, $class, $raw_text, $format = '%PAGE_NUMBER%' ) {
		...
		$text = str_replace( $format, ((($page-1) * $posts_per_page) + 1 )." - ".($page * $posts_per_page), $raw_text );
