...
<p>
     ...
     <select name="<?php print $this->get_field_name('items'); ?>" id="<?php print $this->get_field_id('items'); ?>">
          <?php
          for ( $i = 1; $i <= 20; ++$i ) print "<option value='$i' " . selected($instance['items'], $i, true) . ">$i</option>";
          ?>
     </select>
     <label for="<?php print $this->get_field_id('rss_url'); ?>">
          <?php _e('items from', 'better-rss-widget'); ?>
     </label>
     <input style="width:350px;" id="<?php print $this->get_field_id('rss_url'); ?>" name="<?php print $this->get_field_name('rss_url'); ?>" type="text" value="<?php print $instance['rss_url']; ?>" />
</p>
<p>
