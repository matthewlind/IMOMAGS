...
function custom_terms_checklist($post_id = 0, $args = array()) {
	        ...
	        $galleries = get_term_by( 'slug', 'galleries', 'category' );
	       	...
	       	$gunsmith = get_term_by( 'slug', 'ask-the-gunsmith', 'category' );

		    // Create the ID array
		    $include = array(
		    	$featured->term_id,$galleries->term_id,$video->term_id,$gunsmith->term_id,
...
function custom_gun_terminology_checklist($post_id = 0, $args = array()) {
        ...
        $ak47 = get_term_by( 'slug', 'ak47', 'category' );
        ...
        $bullpup = get_term_by( 'slug', 'bullpup', 'category' );
	    ...
	    $include = array(
	        ...
	        $ballistics->term_id,$long_guns->term_id,$muzzleloaders->term_id,$reloading->term_id,$ak47->term_id,$bullpup->term_id
	    );
...
function custom_topics_checklist($post_id = 0, $args = array()) {
		    ...
		    $second_amendment = get_term_by( 'slug', 'second-amendment', 'category' );
		    ...
		    // Create the ID array
		    $include = array(
		    	$reloading->term_id,$military->term_id,$historical->term_id,$long_guns->term_id,
		        $personal->term_id,$reloading->term_id,$survival->term_id,$gunsmithing->term_id,
		        $competition->term_id,$politics->term_id,$carry->term_id,$second_amendment->term_id,
		        $hunting->term_id,$zombies->term_id,$tactical->term_id,$culture->term_id
		    );
