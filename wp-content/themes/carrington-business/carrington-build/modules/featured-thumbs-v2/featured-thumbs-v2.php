...
if (!class_exists('cfct_module_featured_thumbs_v2') && class_exists('cfct_build_module')) {
	class cfct_module_featured_thumbs_v2 extends cfct_build_module {
		...
		public function __construct() {
			$opts = array(
				'description' => __('A 4 thumb Image Carousel (search post titles only)', 'carrington-build'),
				'icon' => 'featured-thumbs/icon.png'
			);
			parent::__construct('cfct-module-featured-thumbs-v2', __('Featured Thumbs v2', 'carrington-build'), $opts);
			add_filter('wp_ajax_cfct_featured_thumbs_v2_post_search', array($this, '_handle_featured_thumbs_v2_request'));
			...
			$js = preg_replace('/^(\t){4}/m', '', '
				...
				cfct_builder.addModuleLoadCallback("'.$this->id_base.'", function() {
					...
					$("#car-item-search #car-search-term").oTypeAhead({
						searchParams: {
							action: "cfct_featured_thumbs_v2_post_search",
...
// Search Request
		...
		public function _handle_featured_thumbs_v2_request() {
...
/*			$s = new WP_Query(array(
				...
				'featured_thumbs_posts_title' => $_POST['car-search-term']

 			)); */
			$term = $_POST['car-search-term'];
			$querystr = "SELECT SQL_CALC_FOUND_ROWS  ".$wpdb->posts.".* FROM ".$wpdb->posts."  INNER JOIN wp_13_postmeta ON (".$wpdb->posts.".ID = wp_13_postmeta.post_id) WHERE 1=1  AND (((".$wpdb->posts.".post_title LIKE '%".$term."%') ))  AND ".$wpdb->posts.".post_type IN ('post', 'page', 'cfct-news') AND (".$wpdb->posts.".post_status = 'publish' OR ".$wpdb->posts.".post_status = 'future' OR ".$wpdb->posts.".post_status = 'draft' OR ".$wpdb->posts.".post_status = 'pending' OR ".$wpdb->posts.".post_status = 'private') AND (wp_13_postmeta.meta_key = '_thumbnail_id' ) GROUP BY ".$wpdb->posts.".ID ORDER BY ".$wpdb->posts.".post_date DESC LIMIT 0, 8";

			$s = $wpdb->get_results($querystr);
...
// Content Move Helpers
	...
	cfct_build_register_module('cfct_module_featured_thumbs_v2');
}
?>