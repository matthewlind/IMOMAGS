 ...
 */
if (!class_exists('cfct_module_imo_slider') && class_exists('cfct_build_module')) {
	class cfct_module_imo_slider extends cfct_build_module {
		const POST_TYPES_FILTER = 'cfct-module-loop-post-types';
		const TAXONOMY_TYPES_FILTER = 'cfct-module-loop-taxonomy-types';
		
		protected $_deprecated_id = 'cfct-module-imo-slider'; // deprecated property, not needed for new module development
		...
		public function __construct() {
			$opts = array(
				...
				'icon' => 'imo-slider/icon.png'
			);
			parent::__construct('cfct-module-imo-slider', __('IMO Slider', 'carrington-build'), $opts);
			
			
			wp_register_script('jcarousel', $this->get_url().'js/jquery.jcarousel.min.js', array('jquery'));
			
			wp_register_style('imo-slider-style',$this->get_url().'css/imo-slider.css');
			
			if (!is_admin()) {
				//wp_enqueue_script('jquery-cycle');
				wp_enqueue_script('jcarousel');
				wp_enqueue_style('imo-slider-style');
...
# Output
				...
				while ($query->have_posts()) {
					...
					if ($count == 1) {
						...
						<div id="imo-slider">
							...
							<ul id="krang-carousel" class="jcarousel-skin-krang">
		...
		function the_featured_slider_block() {   
			...
			<li class="slider-item">
				...
				<div class="carousel-cell" style="height:125px;width:110px;">
					<div class="image">
					...
					<h3>
						<a href="<?php the_permalink(); ?>"><?php $this->the_title_max_charlength(43); ?></a>
					</h3>
				</div>
...
// Content Move Helpers
	...
	cfct_build_register_module('cfct_module_imo_slider');
}
?>