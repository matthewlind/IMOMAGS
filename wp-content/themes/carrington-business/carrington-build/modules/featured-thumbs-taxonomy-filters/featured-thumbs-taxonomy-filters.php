...
if (!class_exists('cfct_module_featured_thumbs_taxonomy_filters') && class_exists('cfct_build_module')) {
	class cfct_module_featured_thumbs_taxonomy_filters extends cfct_build_module {
		...
		public function __construct() {
			$opts = array(
				'description' => __('A 4 thumb Image Carousel (with taxomomy filters)', 'carrington-build'),
				'icon' => 'featured-thumbs-taxonomy-filters/icon.png'
			);
			parent::__construct('cfct_module_featured_thumbs_taxonomy_filters', __('Featured Thumbs (taxonomy filters)', 'carrington-build'), $opts);
			add_filter('wp_ajax_cfct_module_featured_thumbs_taxonomy_filters_post_search', array($this, '_handle_cfct_module_featured_thumbs_taxonomy_filters_request'));
			
			wp_register_script('featured-thumbs-taxonomy-filters-js',$this->get_url().'js/featured-thumbs-taxonomy-filters.js',array('jquery','jquery-ui-core','jquery-ui-tabs'));
			wp_register_style('featured-thumbs-taxonomy-filters-css',$this->get_url().'css/featured-thumbs-taxonomy-filters.css');
...
// Display
		...
		public function display($data) {
			...
			while ( $the_query->have_posts() ) : $the_query->the_post();
				$ft_id = get_the_id();
				$data['cfct-module-featured-thumbs-taxonomy-filters-posts'][$ft_id] = array( 'id' => $ft_id, 'title' => get_the_title(), 'content' => '' );
			endwhile;
...
# Admin Ajax
		...
		public function admin_css() {
			return preg_replace('/^(\t){4}/m', '', '
				...
				.cfct-popup-content #car-item-search .otypeahead-target .cfct-module-featured-thumbs-taxonomy-filters-loading {
					padding: 5px;
					font-size: .9em;
					color: gray;
					-moz-border-radius-bottomleft: 5px; /* FF1+ */
			...
			$js = preg_replace('/^(\t){4}/m', '', '
				...
				cfct_builder.addModuleLoadCallback("'.$this->id_base.'", function() {
					...
					$("#car-item-search #car-search-term").oTypeAhead({
						searchParams: {
							action: "cfct_featured_thumbs_taxonomy_filters_post_search",
...
// Search Request
		...
		public function _handle_featured_thumbs_taxonomy_filters_request() {
...
// Content Move Helpers
	...
	cfct_build_register_module('cfct_module_featured_thumbs_taxonomy_filters');
