 ...
 *
 * Don't forget to call not_loop::init() in your constructor if you
 * derive from this class!
 */
if (!class_exists('not_loop') && class_exists('cfct_build_module')) {
	class not_loop extends cfct_build_module {
		const POST_TYPES_FILTER = 'not-loop-post-types';
		const TAXONOMY_TYPES_FILTER = 'not-loop-taxonomy-types';
		
		protected $_deprecated_id = 'not-loop'; // deprecated property, not needed for new module development
		...
		public function __construct() {
			$opts = array(
				'description' => __('A regular loop that only shows NOT Featured posts.', 'carrington-build'),
				'icon' => 'not-loop/icon.png'
			);
			parent::__construct('not-loop', __('NOT Loop', 'carrington-build'), $opts);
			$this->init();
		}
...
# Output
		...
		protected function set_display_args($data) {
			...
			//Don't include FEATURED posts because this is a NOT loop.
			...
			$args['category__not_in'] = array($tid);
...
// Content Move Helpers
	...
	cfct_build_register_module('not_loop');
}
?>