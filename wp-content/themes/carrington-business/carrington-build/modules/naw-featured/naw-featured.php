 ...
 */
if (!class_exists('cfct_module_naw_featured') && class_exists('cfct_build_module')) {
	class cfct_module_naw_featured extends cfct_build_module {
		const POST_TYPES_FILTER = 'cfct-module-loop-post-types';
		const TAXONOMY_TYPES_FILTER = 'cfct-module-loop-taxonomy-types';
		
		protected $_deprecated_id = 'cfct-module-naw-featured'; // deprecated property, not needed for new module development
		...
		public function __construct() {
			$opts = array(
				'description' => __('A featured items slider that just for NAW.', 'carrington-build'),
				'icon' => 'naw-featured/icon.png'
			);
			parent::__construct('cfct-module-naw-featured', __('NAW Featured', 'carrington-build'), $opts);
			
			
			wp_register_script('jcarousel', $this->get_url().'js/jquery.jcarousel.min.js', array('jquery'));
			
			wp_register_style('naw-featured-style',$this->get_url().'css/naw-featured.css');
...
# Output
			...
			if ($query->have_posts()) {
				...
				while ($query->have_posts()) {
					...
					if ($count == 1) {
						...
						?>
						<div id="fs-featured" class="naw-featured">
							<div class="left"> 
								<ul id="fs-feature-carousel" class="jcarousel-skin-naw-featured" style="height:404px;">
		...
		function the_featured_slider_block() {   
			...
			<li class="featured-item">
				...
				<div class="naw-feature-image"> 
					<a href="<?php the_permalink(); ?>"><?php echo get_the_post_thumbnail(get_the_ID(),"medium-thumb"); ?></a>
				</div>
		
				<div class="featured-item-description naw-desc">
...
var custom_feature_config = {};
	...
	$(".jcarousel-skin-naw-featured .jcarousel-next-horizontal").css("margin-right",rightMargin + "px");
	$(".jcarousel-skin-naw-featured .jcarousel-prev-horizontal").css("margin-left",leftMargin + "px");
	...
	setupCallback:init_scrolldots
      ...
      $("#naw-feature-carousel").jcarousel(feature_config);
...
// Content Move Helpers
	...
	cfct_build_register_module('cfct_module_naw_featured');
}
?>