...
if (!class_exists('cfct_module_featured_thumbs_with_taxonomy') && class_exists('cfct_build_module')) {
	class cfct_module_featured_thumbs_with_taxonomy extends cfct_build_module {
		...
		public function __construct() {
			$opts = array(
				'description' => __('A 4 thumb Image Carousel', 'carrington-build'),
				'icon' => 'featured-thumbs-with-taxonomy/icon.png'
			);
			parent::__construct('cfct-module-featured-thumbs-with-taxonomy', __('Featured Thumbs v2', 'carrington-build'), $opts);
			add_filter('wp_ajax_cfct_featured_thumbs_with_taxonomy_post_search', array($this, '_handle_featured_thumbs_with_taxonomy_request'));
			
			wp_register_script('featured-thumbs-with-taxonomy-js',$this->get_url().'js/featured-thumbs-with-taxonomy.js',array('jquery','jquery-ui-core','jquery-ui-tabs'));
			wp_register_style('featured-thumbs-with-taxonomy-css',$this->get_url().'css/featured-thumbs-with-taxonomy.css');
			
			if (!is_admin()) {
				
				wp_enqueue_script('jquery-ui-tabs');
				wp_enqueue_script('jquery-ui-core');
				wp_enqueue_script('featured-thumbs-with-taxonomy-js',NULL,array('jquery','jquery-ui-core','jquery-ui-tabs'));
				wp_enqueue_style('featured-thumbs-with-taxonomy-css');
			}
			...
			$this->transitions = apply_filters('cfct-featured-thumbs-with-taxonomy-transitions-options', array(
			...
			$this->nav_positions = apply_filters('cfct-featured-thumbs-with-taxonomy-nav-positions', array(
...
// Display
		...
		public function display($data) {
			...
			$car_opts = array(
				...
				'nav_element' => apply_filters('cfct-featured-thumbs-with-taxonomy-nav-element', '<div class="car-pagination"><ol></ol></div>'),
				'nav_selector' => apply_filters('cfct-featured-thumbs-with-taxonomy-nav-selector', '#featured-thumbs-with-taxonomy-'.$data['module_id'].' .car-pagination ol', '#featured-thumbs-with-taxonomy-'.$data['module_id'])
			);
			
			// Make sure you quote string values - this distinguishes them from object literals
			$js_opts = apply_filters('cfct-featured-thumbs-with-taxonomy-js-options', array(
				...
				'pause' => 'true',
				'prev' => '$(\'<a class="cfct-featured-thumbs-with-taxonomy-prev">'.__('Prev', 'carrington-build').'</a>\').insertBefore("'.$car_opts['nav_selector'].'")',
				'next' => '$(\'<a class="cfct-featured-thumbs-with-taxonomy-next">'.__('Next', 'carrington-build').'</a>\').insertAfter("'.$car_opts['nav_selector'].'")',
			...
			$js_init = apply_filters('cfct-featured-thumbs-with-taxonomy-js-init', '
			<script type="text/javascript">
				jQuery(function($) {
					$("#featured-thumbs-with-taxonomy-'.$data['module_id'].' .car-content ul").cycle('.$jobj.');
				});
...
// Admin
			...
			$html .= '
					...
					<div id="car-items" class="active">
						...
						<div class="car-items-wrapper">
							<ol class="featured-thumbs-with-taxonomy-list">';
				if (count($data[$this->get_field_name('posts')])) {
					foreach ($data[$this->get_field_name('posts')] as $item) {
						$html .= $this->get_featured_thumbs_with_taxonomy_admin_item($item);
					}
				}
				else {
					$html .= '
		...
		public function admin_css() {
			return preg_replace('/^(\t){4}/m', '', '
				.featured-thumbs-with-taxonomy-sortable-placeholder {
					height: 18px;
				...
				/* Carousel List */
				.featured-thumbs-with-taxonomy-list {
				...
				.featured-thumbs-with-taxonomy-list li {
				...
				.featured-thumbs-with-taxonomy-list li:hover {
					background: #fff url('.$this->get_url().'img/featured-thumbs-with-taxonomy-drag.gif) 100% 50% no-repeat;
					cursor: move;
				}
				.featured-thumbs-with-taxonomy-list li.featured-thumbs-with-taxonomy-item-edit:hover {
					background: none;
				}
				.featured-thumbs-with-taxonomy-list li:first-child {
				...
				.featured-thumbs-with-taxonomy-list li:last-child {
					border-bottom: 0;
				...
				/* clearfix */
				.featured-thumbs-with-taxonomy-list li:after { content: "."; display: block; height: 0; clear: both; visibility: hidden; }

				/* Floating */
				.featured-thumbs-with-taxonomy-item-img,
				.featured-thumbs-with-taxonomy-item-title,
				.featured-thumbs-with-taxonomy-edit-form {
					float: left;
				}
				...
				.featured-thumbs-with-taxonomy-item-img {
					background: #D2CFCF url('.$this->get_url().'img/featured-thumbs-with-taxonomy-none-icon.png) center center no-repeat;
				...
				.featured-thumbs-with-taxonomy-item-edit .featured-thumbs-with-taxonomy-item-title {
					display: none;
				}
				.featured-thumbs-with-taxonomy-item-edit .featured-thumbs-with-taxonomy-item-img {
					height: 150px;
				...
				.featured-thumbs-with-taxonomy-item-edit .featured-thumbs-with-taxonomy-edit-form {
					display: block;
				}
				...
				.featured-thumbs-with-taxonomy-edit-form label {
					display: none;
				}
				.featured-thumbs-with-taxonomy-edit-form input.text,
				.featured-thumbs-with-taxonomy-edit-form textarea {
					width: 90%;	
				}
				.featured-thumbs-with-taxonomy-edit-form input.text {
					font-size: 13px;
				...
				.featured-thumbs-with-taxonomy-edit-done {
					margin-top: 7px;
				}
				.featured-thumbs-with-taxonomy-edit-remove {
					line-height: 1px;
					margin: 0 0 0 10px;
				}			
				...
				.cfct-popup-content #car-item-search .otypeahead-target li .featured-thumbs-with-taxonomy-item-title,
				...
				.cfct-popup-content #car-item-search .otypeahead-target .cfct-module-featured-thumbs-with-taxonomy-loading {
					padding: 5px;
					font-size: .9em;
					color: gray;
					-moz-border-radius-bottomleft: 5px; /* FF1+ */
				...
				.cfct-popup-content #car-item-search .otypeahead-target li .featured-thumbs-with-taxonomy-item-img {
					display: none;
				}
			');
		}
			...
			$js = preg_replace('/^(\t){4}/m', '', '
				...
				cfct_builder.addModuleLoadCallback("'.$this->id_base.'", function() {
					...
					$("#car-item-search #car-search-term").oTypeAhead({
						searchParams: {
							action: "cfct_featured_thumbs_with_taxonomy_post_search",
					...
					$("#car-items ol").sortable({
						...
						opacity: 0.6,
						containment: "parent",
						placeholder: "featured-thumbs-with-taxonomy-sortable-placeholder"
					});
				});
			
				var '.$js_base.'_insert_selected_item = function(_insert) {
					$("#car-items ol").append(_insert).find(".no-items").hide().end().sortable("refresh");
					$("a.featured-thumbs-with-taxonomy-post-item-ident", _insert).trigger("click");
					$("body").trigger("click");
				...
				// set up post edit
				$("#car-items li.featured-thumbs-with-taxonomy-post-item .featured-thumbs-with-taxonomy-post-item-ident, #car-items li.featured-thumbs-with-taxonomy-post-item .featured-thumbs-with-taxonomy-item-img").live("click", function() {
					$(this).closest(".featured-thumbs-with-taxonomy-post-item").addClass("featured-thumbs-with-taxonomy-item-edit");
					return false;
				});
								
				// set up post done edit
				$("#car-items li.featured-thumbs-with-taxonomy-post-item .featured-thumbs-with-taxonomy-edit-done").live("click", function() {
					$(this).closest(".featured-thumbs-with-taxonomy-post-item").removeClass("featured-thumbs-with-taxonomy-item-edit");
					return false;
				});
								
				// set up post remove
				$("#car-items li.featured-thumbs-with-taxonomy-post-item .featured-thumbs-with-taxonomy-edit-remove a").live("click", function() {
					if (confirm("Do you really want to remove this item?")) {
						$(this).closest(".featured-thumbs-with-taxonomy-post-item").remove();
		 ...
		 * @return string HTML
		 */
		protected function get_featured_thumbs_with_taxonomy_admin_item($postdata) {
			$img = array();
			...
			$html = '
				<li class="featured-thumbs-with-taxonomy-post-item">
					<div class="featured-thumbs-with-taxonomy-item-img"'.$img_style.'></div>
					<a class="featured-thumbs-with-taxonomy-post-item-ident featured-thumbs-with-taxonomy-item-title" href="#featured-thumbs-with-taxonomy-post-'.intval($postdata['id']).'">'.esc_html($postdata['title']).'</a>
					<div class="featured-thumbs-with-taxonomy-edit-form">
						...
						<input type="button" name="done" class="button featured-thumbs-with-taxonomy-edit-done" value="Done" />
						<span class="featured-thumbs-with-taxonomy-edit-remove trash"><a href="#remove" class="lnk-remove">remove</a></span>
					</div>
				</li>
				';			
			return $html;
...
// Search Request
		...
		public function _handle_featured_thumbs_with_taxonomy_request() {
			if (!empty($_POST['carousel_action'])) {
				switch($_POST['carousel_action']) {
					case 'do_search':
						$this->_featured_thumbs_with_taxonomy_do_search();
						break;
				}
				exit;
			}
		...
		protected function _featured_thumbs_with_taxonomy_do_search() {
			// ONLY PULLS POSTS THAT HAVE A FEATURED IMAGE
			$s = new WP_Query(array(
				's' => $_POST['car-search-term'],
				'post_type' => apply_filters('cfct-featured-thumbs-with-taxonomy-search-in', array_filter(get_post_types(array('public' => 1)), array($this, 'filter_post_types'))),
				'posts_per_page' => 8,
			...
			if ($s->have_posts()) {
				...
				while ($s->have_posts()) {
					...
					$html .= $this->get_featured_thumbs_with_taxonomy_admin_item($postdata);
				}
				$html .= '</ul>';
				$ret['html'] = $html;
			}
...
// Content Move Helpers
	...
	cfct_build_register_module('cfct_module_featured_thumbs_with_taxonomy');
}
?>