...
class WPB_Widget_Tabs extends WP_Widget {
	...
	public function __construct() {
		parent::__construct($this->id,$this->name,
			...
			array(
				...
				'description'	=> __('List posts, comments, and/or tags with or without tabs.','air')
			)
		);
	}
	...
	**/
 	public function form($instance) {
		...
		$form .= AirForm::widget_select($this->_field_atts('recent-num',array('class'=>'')), $val['recent-num'],
			air_range(0,10), __('Number of posts: ','air'));

		// Divider
		$form .= '<hr style="border:none;border-bottom: 2px solid #ddd;">';

		// Most Popular
		$form .= sprintf('<h4 style="margin-bottom:0.665em">%s</h4>',__('Most Popular','air'));
		$form .= AirForm::widget_checkbox($this->_field_atts('popular-enable'), $val['popular-enable'],
			__('Enable most popular','air'), '<p>', '<br/>');
		...
		$form .= AirForm::widget_select($this->_field_atts('popular-time'), $val['popular-time'],
			array(
				'0'		=> __('All Time','air'),
				'month'	=> __('This Month', 'air'),
				'week'	=> __('This Week', 'air'),
				'day'	=> __('Past 24 hours','air')
			), __('Posts with most comments from:','air'));
		$form .= AirForm::widget_select($this->_field_atts('popular-num',array('class'=>'')), $val['popular-num'],
			air_range(0,11), __('Number of posts: ','air'));

		// Divider
		$form .= '<hr style="border:none;border-bottom: 2px solid #ddd;">';

		// Recent Comments
		$form .= sprintf('<h4 style="margin-bottom:0.665em">%s</h4>',__('Recent Comments','air'));
		$form .= AirForm::widget_checkbox($this->_field_atts('comments-enable'), $val['comments-enable'],
			__('Enable recent comments','air'), '<p>', '<br/>');
		$form .= AirForm::widget_checkbox($this->_field_atts('comments-avatar'), $val['comments-avatar'],
			__('Enable avatars','air'), '');
		...
		// Tags
		$form .= sprintf('<h4 style="margin-bottom:0.665em">%s</h4>',__('Tags','air'));
		...
		// Tab Order
		$form .= sprintf('<h4 style="margin-bottom:0.665em">%s</h4>',__('Tab Order','air'));
		
		$form .= '<table>';
		
		$form .= sprintf('<tr><td style="width:125px;">%s</td>',__('Recent Posts','air'));
		$form .= sprintf('<td>%s</td></tr>',
			AirForm::text($this->_field_atts('order-recent',array('size'=>2)),
			$val['order-recent']));
		
		$form .= sprintf('<tr><td>%s</td>',__('Most Popular','air'));
		$form .= sprintf('<td>%s</td></tr>',
			AirForm::text($this->_field_atts('order-popular',array('size'=>2)),
			$val['order-popular']));
		
		$form .= sprintf('<tr><td>%s</td>',__('Comments','air'));
		$form .= sprintf('<td>%s</td></tr>',
			AirForm::text($this->_field_atts('order-comments',array('size'=>2)),
			$val['order-comments']));

		$form .= sprintf('<tr><td>%s</td>',__('Tags','air'));
		$form .= sprintf('<td>%s</td></tr>',
	...
	**/
	public function update($new,$old) {
		...
		$options = 'title|recent-enable|recent-cat-id|recent-thumb|recent-num'.
			'|popular-enable|popular-time|popular-thumb|popular-num'.
	...
	**/
	private function _create_tabs($tabs,$count,$wid) {
		...
		// Loop through tabs
		foreach ( $tabs as $tab ) {
			$output .= sprintf('<li class="wpb-tab-%1$s"><a href="#wpb-tab-%2$s"><span>%3$s</span></a></li>',
				$tab, $tab.'-'.$wid, ucfirst($tab));
		}
	...
	**/
	private function _popular($opts,$wid) {
		...
		foreach ( $posts as $post ) {
			...
			// $output .= sprintf('<i class="meta">%1$s %2$s</i>', $comments_num, __('comments','air'));
			$output .= '</span></a></li>';
		}
	...
	private function _comments($opts,$wid) {
		...
		foreach ( $comments as $comment ) {
			...
			$output .= sprintf('<i class="meta">%1$s %2$s</i>',
				$comment->comment_author, __('on:','air'));
			$output .= sprintf('<i class="title">%s</i>', $title);
			$output .= '</span></a></li>';
		}
